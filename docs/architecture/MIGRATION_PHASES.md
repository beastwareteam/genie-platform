...
3. **Facade aufbauen** — Implementierung in `application/facade/` mit Orchestration (QueryEngine + TaskOrchestrator + EventBus)
4. **ViewModel entkoppeln** — viewmodels konsumieren nur Interfaces, nie direkt QueryEngine/TaskOrchestrator
5. **Container-Wiring trennen** — separate DI-Profile für Core-Komposition und Edge-Komposition (Desktop zuerst)
6. **Server-Edge vorbereiten** — HTTP/WebSocket-Adapter gegen Facade-Interfaces (Stubs zuerst)
7. **Quality Gates aktivieren** — Interface-Contract-Tests, Linter-Regel für Edge→Interfaces
8. **Review** — Architektur-Review: Interface-Contract stabil? Dann Freeze aufheben oder weiter fortsetzen.
