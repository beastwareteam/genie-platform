...
- **Innen (Provider):** \pplication/interfaces/*\ (reine Protocol-Definitionen)
...
|---------|--------------|----------------|
| \pplication/interfaces/\ | Protocol-Definitionen (ChatService, TaskService, SessionService) | keine |
| \pplication/facade/\ | Service-Implementierung, Orchestration-Leitung | interfaces, QueryEngine, TaskOrchestrator, EventBus, Domain (read) |
| \presentation/viewmodels/\ | UI-State-Management, Command-Dispatch | application/interfaces, shared |
| \pp/container.py\ | Dependency Injection für Core + Edge (DI-Profile getrennt) | alle |
...
- Interface-Contract-Tests für jedes Service-Interface in \pplication/interfaces/\
- Keine direkten Imports von QueryEngine/TaskOrchestrator außerhalb der Facade
- Edge (Desktop/Server) importiert nur aus \pplication/interfaces\ und \shared\
- Domain-Zugriff nur lesend (via Ports), keine neue Logik
- Linter-Regel: \pplication/viewmodels\ nur \pplication/interfaces\ erlaubt
